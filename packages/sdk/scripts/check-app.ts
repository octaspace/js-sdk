...
const client = new OctaClient({ apiKey, retries: 0 })
...
for (const m of sorted) {
  console.log(
    `  node ${m.node_id} | ${m.city}, ${m.country} | ${m.cpu_cores}c / ${(m.total_memory / 1024 ** 3).toFixed(0)}GB | disk_free: ${(m.free_disk / 1024 ** 3).toFixed(0)}GB | $${m.total_price_usd.toFixed(5)}/hr | gpu: ${m.is_has_gpu ? m.gpus.map((g) => g.model).join(',') : 'none'}`,
  )
}
